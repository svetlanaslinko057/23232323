  ...
  const fetchMarkets = async () => {
    try {
      ...
      if (activeFilter === 'volume') {
        params.append('sortBy', 'totalVolume');
        params.append('sortOrder', 'desc');
      } else if (activeFilter === 'closing') {
        params.append('sortBy', 'endTime');
        params.append('sortOrder', 'asc');
      }
      
      const response = await fetch(`${apiUrl}/api/onchain/markets?${params}`);
      ...
      if (result.data && result.data.length > 0) {
        ...
        const fallbackResponse = await fetch(`${apiUrl}/api/predictions?status=active`);
        const fallbackResult = await fallbackResponse.json();
        if (fallbackResult.data && fallbackResult.data.length > 0) {
          const transformed = fallbackResult.data.map(transformMarket);
          setMarkets(transformed);
        } else {
