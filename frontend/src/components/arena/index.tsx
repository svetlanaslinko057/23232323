...
// Dynamic imports for components using wagmi/rainbowkit hooks
const ProfileDropdown = dynamic(
  () => import("@/components/wallet/ProfileDropdown").then(mod => mod.ProfileDropdown),
  { ssr: false, loading: () => <div style={{ width: 120, height: 40 }} /> }
);

const CreatePredictionModal = dynamic(
  () => import("./create-prediction-modal/CreatePredictionModal").then(mod => mod.CreatePredictionModal),
  { ssr: false }
);

const CreateDuelModal = dynamic(
  () => import("./create-duel-modal/CreateDuelModal").then(mod => mod.CreateDuelModal),
  { ssr: false }
);
